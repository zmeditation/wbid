...
module.exports = async (req, res) => {
    ...
    if (userConfigs.disabled === true) {
        console.log('This site already disabled');
        return res.status(200).send('This site already disabled');
    }
    let toDisable = [];
        ...
        for (let config of userConfigs['configs']) {
            ...
            for (let adapter of JSON.parse(body.adaptersList)) {
                if (settings[adapter]['disabled']) {
                    ...
                    settings[adapter]['disabled'] = {data: true, type: ""};
                }
            }
            body.settings = JSON.stringify(settings);
            toDisable.push(fetch('http://localhost:9999/editPrebidConfig', {
                method: 'post',
                body: JSON.stringify(body),
                headers: {'Content-Type': 'application/json'},
            }));
        }
        Promise.allSettled(toDisable)
            .then((result) => {
                let counter = 0;
                for (let r of result) {
                ...
                if (counter === 0) {
                    SiteDB.update(id, undefined, undefined, true);
                    res.status(200).send('All configs was disabled');
